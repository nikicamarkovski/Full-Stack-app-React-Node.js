...
const { AgeValidator, EmailValidator, PasswordValidator, SamePassword , loginLogic , Email} = require('../helper');
    ...
    try {
        ...
        if (Email(isExist)) {
            ...
            if (EmailValidator(body.email) && AgeValidator(body.age) && PasswordValidator(body.password)) {
                ...
                let error = new Error('Check your age , Email and password')
                error.status = 401;
                next(error)
            }
        } else {
...
DetelePatient = async (req, res) => {
    try {
        ...
        if (tokenData.user.doctor_id == patient[0].doctor_id) {
            ...
            var error = new Error("Not your patient");
            error.status = 404;
           throw(error);
        }
    } catch (error) {
